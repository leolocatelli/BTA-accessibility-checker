...
function sanitize(html) {
  return sanitizeHtml(html, {
    ...
    // 🔹 adiciona isto para silenciar o warning sobre tags "vulneráveis" (como <style>)
  ...
  return page.evaluate(
    ({ scope, includeDialogs, FOCUSABLE, OVERLAY_CSS }) => {
      ...
      try {
        ...
        kill(
          "#js-disabled, .js-disabled, .no-js-banner, [data-js-disabled], [data-noscript], .no-js-msg"
        );
        const rx =
          /javascript (is|functionality is) turned off|enable javascript/i;
        for (const el of document.body.querySelectorAll("*")) {
          try {
            if (!el.childElementCount && rx.test(el.textContent || ""))
              el.closest("div,section,header,aside")?.remove();
          } catch {}
        }
      } catch {}
      ...
      try {
        const rmAll = (sel) =>
          document.querySelectorAll(sel).forEach((n) => n.remove());
        rmAll(
          '#onetrust-banner-sdk, #onetrust-consent-sdk, #onetrust-pc-sdk, .ot-sdk-container, .ot-sdk-overlay, iframe[title="onetrust-text-resize"]'
        );
        rmAll(
          '.cookie-banner, .cookie-consent, [id*="cookie" i], [class*="cookie" i], [id*="consent" i], [class*="consent" i]'
        );
      ...
      const roots = [
        scope === "full"
          ? document.body
          : mainById || mainTag || mainRole || document.body,
      ];
      if (includeDialogs)
        document
      ...
      const shouldIgnore = (el) =>
        scope !== "full" &&
        el.closest(
          "header, footer, [role='banner'], [role='contentinfo'], .site-header, .site-footer"
        );

      const candidates = roots.flatMap((r) =>
        Array.from(r.querySelectorAll(FOCUSABLE))
      );
      ...
      const withMeta = focusables
        .map((el, i) => {
          ...
          const tabIndex = ti ? parseInt(ti, 10) : 0;
          return {
            el,
            id,
            tabIndex,
        ...
        .sort((a, b) => {
          const ap = a.tabIndex > 0 ? 0 : 1,
            bp = b.tabIndex > 0 ? 0 : 1;
          return (
            ap - bp ||
            (ap === 0
              ? a.tabIndex - b.tabIndex || a.domIndex - b.domIndex
      ...
      return {
        ...
        summary: {
          ...
          noName: focusablesPayload.filter(
            (f) => !f.accessibleName || !f.accessibleName.trim()
          ).length,
...
// 🔹 AGORA SUPORTA mode + html
async function inspect(url, opts) {
  const mode = (opts && opts.mode) || "url";
  const htmlSnapshot = opts && opts.html;
  ...
  try {
    ...
    page.on("request", (req) => {
      ...
      if (BLOCK_HOSTS.some((h) => lowerUrl.includes(h))) return req.abort();
      return req.continue();
    });
    ...
    if (mode === "html") {
      // 🔹 MODO SNAPSHOT: injeta o HTML recebido em vez de navegar para uma URL
      if (!htmlSnapshot || typeof htmlSnapshot !== "string") {
        throw new Error("Missing HTML snapshot for mode 'html'");
      }
      try {
        await page.setContent(htmlSnapshot, {
          waitUntil: ["load", "domcontentloaded"],
          timeout: 12000,
        });
      } catch (e) {
        console.warn("setContent warning:", e.message);
      }
    } else {
      // 🔹 MODO URL: fluxo normal
      try {
        await page.goto(url, {
          waitUntil: ["load", "domcontentloaded"],
          timeout: 12000,
    ...
    const pageUrlForBase =
      mode === "html" ? url || "about:blank" : url || "about:blank";
    const html = injectBaseHref(sanitize(data.html), pageUrlForBase);
  ...
  app.post("/api/inspect", async (req, res) => {
    ...
    try {
      ...
      if (mode === "url") {
        ...
        return res
          .status(400)
          .json({ error: "Invalid mode. Use 'url' or 'html'" });
      }

      const data = await withTimeout(
        inspect(url, { mode, html, scope, includeDialogs }),
        T_MAX,
        "inspect"
      );
      ...
      return res
        ...
        .json({
          ok: false,
          error: isTimeout
            ? "Request timed out while inspecting the page."
            : msg,
        });
    }
  });
