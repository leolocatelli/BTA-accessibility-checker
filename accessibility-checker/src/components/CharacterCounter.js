...
export default function CharacterCounter({ cta_desc }) {
  ...
  const pushHistory = (val) => setHistory((h) => [...h.slice(-29), val]); // keep last 30
  ...
  // ---- transforms (somente as usadas na UI atual)
  ...
  const toTitleSmart = (s) => {
    ...
    return tokens
      .map((w, i) => (i === 0 || i === tokens.length - 1 || !small.has(w))
  ...
  const toCamel = (s) => {
    ...
    return w[0] + w.slice(1).map((t) => t.charAt(0).toUpperCase() + t.slice(1)).join("");
  };
  const toPascal = (s) => wordsFromText(s).map((t) => t.charAt(0).toUpperCase() + t.slice(1)).join("");
  ...
  const transforms = {
    ...
    title: (txt) => txt.replace(/\w\S*/g, (w) => w[0].toUpperCase() + w.slice(1).toLowerCase()),
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (k === "u") { e.preventDefault(); updateText(transforms.upper); announce("Converted to UPPERCASE"); }
      if (k === "l") { e.preventDefault(); updateText(transforms.lower); announce("Converted to lowercase"); }
      if (k === "t") { e.preventDefault(); updateText(transforms.titleSmart); announce("Converted to Title Case"); }
      if (k === "k") { e.preventDefault(); updateText(transforms.slug); announce("Converted to slug-case"); }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow-md">
      ...
      <div className="flex flex-wrap gap-2 justify-center mb-6">
        <button onClick={() => updateText(transforms.snake)} className={buttonStyle} aria-label="snake_case" title="snake_case">
          snake_case
        </button>
        <button onClick={() => updateText(transforms.camel)} className={buttonStyle} aria-label="camelCase" title="camelCase">
          camelCase
        </button>
        <button onClick={() => updateText(transforms.pascal)} className={buttonStyle} aria-label="PascalCase" title="PascalCase">
          PascalCase
        </button>
        <button onClick={() => updateText(transforms.removeExtraSpaces)} className={buttonStyle} aria-label="Remove extra spaces" title="Remove extra spaces">
          <Eraser className="w-4 h-4" /> Spaces
        </button>
      </div>

      {/* controls */}
      <div className="flex flex-wrap gap-2 justify-center mb-6">
        <button onClick={undo} disabled={!history.length} className={ghostBtn} aria-label="Undo" title="Undo (Cmd/Ctrl+Z)">
          <Undo2 className="w-4 h-4" /> Undo
        </button>
        <button onClick={redo} disabled={!future.length} className={ghostBtn} aria-label="Redo" title="Redo (Cmd/Ctrl+Y)">
          <Redo2 className="w-4 h-4" /> Redo
        </button>
        <button
          onClick={() => { pushHistory(text); setText(""); setFuture([]); announce("Cleared"); }}
          className={ghostBtn}
        ...
        <button onClick={copyText} className={ghostBtn} aria-label="Copy all text" title="Copy all" disabled={!text}>
          {copied ? <CheckCircle className="w-4 h-4 text-green-600" /> : <Clipboard className="w-4 h-4" />} {copied ? "Copied!" : "Copy All"}
        </button>
        <button onClick={copySlug} className={ghostBtn} aria-label="Copy slug" title="Copy slug" disabled={!text.trim()}>
      ...
      <div className="text-sm text-gray-700 text-center space-y-1">
        <p><strong>Characters:</strong> {stats.chars} &nbsp;•&nbsp; <strong>Chars (no spaces):</strong> {stats.charsNoSpaces}</p>
        <p><strong>Words:</strong> {stats.words} &nbsp;•&nbsp; <strong>Lines:</strong> {stats.lines}</p>
        <p><strong>Size:</strong> {stats.bytes} bytes &nbsp;•&nbsp; <strong>~{stats.readTimeMin} min</strong> read</p>
      </div>
    </div>
  );
}
